...
async def on_socket_raw_receive(msgs):
    ...
    if msg['t']=="INTERACTION_CREATE":
        ...
        if ctx.type==3: # Buttons, the only one we actually need
            ...
            elif ctx.custom_id=='pronoun_roles':
                s_roles=[]
                for i in config_data['pronoun_roles']['roles']:s_roles.append(str(i['id']))
                for r in s_roles:
                    role = guild.get_role(int(r))
                    if r in roles:await member.add_roles(role)
                    else:await member.remove_roles(role)
...
@bot.command(name="load_roles")
@commands.has_permissions(administrator=True)
async def load_roles(ctx):
    """Loads the roles messages"""
    c=0
    comp = []
    for role in config_data['pronoun_roles']['roles']:
    ...
    msg = {
        "content": config_data['pronoun_roles']['message_content'], 
        "components": message.components.action_row([
            message.components.select(
                config_data['pronoun_roles']['place_holder'],
                f"pronoun_roles",
                comp,
                max=len(comp)
            )
...
@bot.command(name="load_waitlist")
@commands.has_permissions(administrator=True)
